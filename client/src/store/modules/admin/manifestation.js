...
const mutations = {
    ...
    filterManifestationsByDate(state) {
        state.manifestations = state.filter.date ? state.manifestations.filter(manifestations => manifestations.doneAt.split('-').slice(0, 2).join('-') == state.filter.date) : state.manifestations.slice();
    },
...
const actions = {
    ...
    async deleteManifestation({
        ...
        commit('setLoading', true)
        Axios.delete(`${config.apiUrl}/manifestations/${id}`).then(() => {
