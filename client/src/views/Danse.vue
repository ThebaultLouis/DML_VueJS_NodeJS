  ...
  <div id>
    <!-- <v-card height="50px">
      <v-bottom-nav
        ...
        shift
      >
      <v-btn v-on:click.native="allDance()" dark>
        <span>Toutes</span>
        <v-icon>fa-align-justify</v-icon>
      </v-btn>

        <v-btn v-on:click.native="beginnerDance()" dark>
          <span>Débutant</span>
          <v-icon>fa-baby</v-icon>
        </v-btn>

        <v-btn v-on:click.native="initiateDance()" dark>
          <span>Novice</span>
          <v-icon>fa-child</v-icon>
        </v-btn>

        <v-btn v-on:click.native="confirmateDance()" dark>
        ...
        <v-card v-for="danse in danses.danses" :key="danse._id" class="mb-4 amber lighten-4">
          <div>
            ...
            <v-list class="amber lighten-4">
              <v-list-tile v-for="danseRevisee in danse._dansesRevisees" class="my-1">
                <span class="headline">{{danseRevisee.name}}</span>
                <v-btn
                  v-if="danseRevisee.youtube"
                  color="red"
                  dark
                  flat
                  :href="danseRevisee.youtube"
                  target="_blank"
                >
                  <v-icon>fa-play-circle</v-icon>
                </v-btn>
    ...
    <div class="text-xs-center">
      <v-pagination v-model="page" :length="totalPage" :total-visible="5" color="#FB8C00" circle></v-pagination>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    page: function() {
      this.getParamUrl(`${config.apiUrl}/danses/${this.page}`, this.param);
    },
    danses: function() {
  ...
  methods: {
    ...
    beginnerDance() {
      ...
      this.getParamUrl(`${config.apiUrl}/danses/1`, this.param);
    },
    initiateDance() {
      this.page = 1;
