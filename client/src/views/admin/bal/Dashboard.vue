    ...
    <div v-else>
      <v-layout class="mb-5">
        <v-flex offset-sm6 sm4 xs10 offset-xs1>
          <v-btn
            @click="$router.push({name: 'adminBalCreate'})"
            color="amber"
            block
          >Ajouter un nouvel événement</v-btn>
        </v-flex>
      </v-layout>
      <v-layout row>
        <v-flex>
      ...
      <div class="pa-1">
        <v-layout column>
          <v-flex v-for="bal in bals" :key="bal._id">
            <BalItem :bal="bal" :admin="true"></BalItem>
          </v-flex>
        </v-layout>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("adminBal/fetchBals");
  },
  computed: mapState({
    loading: state => state.adminBal.loading,
    bals: state => state.adminBal.bals
  }),
  ...
  watch: {
    date: function() {
      this.$store.dispatch("adminBal/filterBalsByDate", this.date);
    }
  }
};
</script>
