      ...
      <v-flex xs12 sm10 offset-sm1 >
        <v-card class="ma-4 orange lighten-4">
          ...
          <v-flex mb-4 offset-sm2 xs12 sm8 >
            <v-date-picker width="100%" locale="fr-FR" v-model="date" :landscape="false" :reactive="true" color="amber lighten-1" :no-title="true"></v-date-picker>
          </v-flex>


          <v-flex xs10 offset-xs1>
            <v-select
            <v-select v-if="this.niveaux.niveaux"
...
export default {
  ...
  methods: {
    addRevision () {
      ...
      var niveaux = this.niveaux.niveaux.filter(niv => this.revision.niveaux.indexOf(niv._id) != -1)
      ...
      if (this.modified) {
        Axios.patch(`${config.apiUrl}/revisions/${this.revision._id}`, this.revision).then(response => {
          this.loading = false
        ...
        Axios.post(`${config.apiUrl}/revisions`, this.revision).then(response => {
          this.loading = false
